#include "pch.h"
#include "..\Header\Lunch.h"
#include "Export_System.h"
#include "Export_Utility.h"

CLunch::CLunch(LPDIRECT3DDEVICE9 pGraphicDev)
	: CItem(pGraphicDev)
{
	m_eItemNum = ITEM_LUNCH;
}
...
HRESULT CLunch::Ready_GameObject(_vec3 vPos)
{
...
_int CLunch::Update_GameObject(const _float& fTimeDelta)
{
...
void CLunch::LateUpdate_GameObject()
{
	Engine::CGameObject::LateUpdate_GameObject();
}

void CLunch::Render_GameObject()
{
...
HRESULT CLunch::Add_Component()
{
	...
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_LunchTexture"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CLunch* CLunch::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CLunch* pLunch = new CLunch(pGraphicDev);

	if (FAILED(pLunch->Ready_GameObject(vPos)))
	{
		Safe_Release(pLunch);
		MSG_BOX("pLunch Create Failed");
		return nullptr;
	}
