#include "pch.h"
#include "..\Header\AccessoryTable.h"
#include "Export_System.h"
#include "Export_Utility.h"

CAccessoryTable::CAccessoryTable(LPDIRECT3DDEVICE9 pGraphicDev)
	: CItem(pGraphicDev)
{
	 

	m_eItemNum = ITEM_ACCESSORY_TABLE;
}
...
HRESULT CAccessoryTable::Ready_GameObject(_vec3 vPos)
{
	...
	m_wItemName = L"장신구 작업대";
	m_wItemExplain[0] = L"반지와 목걸이를 만드는 작업대입니다.";
	// 원래의 Y 위치 저장
	m_fFirstY = vPos.y;
...
_int CAccessoryTable::Update_GameObject(const _float& fTimeDelta)
{
...
void CAccessoryTable::LateUpdate_GameObject()
{
	Engine::CGameObject::LateUpdate_GameObject();
}

void CAccessoryTable::Render_GameObject()
{
...
HRESULT CAccessoryTable::Add_Component()
{
	...
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_AccessoryTableTexture"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CAccessoryTable* CAccessoryTable::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CAccessoryTable* pAccessoryTable = new CAccessoryTable(pGraphicDev);

	if (FAILED(pAccessoryTable->Ready_GameObject(vPos)))
	{
		Safe_Release(pAccessoryTable);
		MSG_BOX("pAccessoryTable Create Failed");
		return nullptr;
	}

	return pAccessoryTable;
}

void CAccessoryTable::Free()
{
