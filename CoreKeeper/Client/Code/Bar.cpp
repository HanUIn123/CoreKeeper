...
CBar::CBar(LPDIRECT3DDEVICE9 pGraphicDev)
	: CItem(pGraphicDev)
{
}
...
HRESULT CBar::Ready_GameObject(MATERIAL _eMaterial, _vec3 vPos)
{
...
_int CBar::Update_GameObject(const _float& fTimeDelta)
{
...
void CBar::LateUpdate_GameObject()
{
...
CBar* CBar::Create(LPDIRECT3DDEVICE9 pGraphicDev, MATERIAL _eMaterial, _vec3 vPos)
{
	CBar* pBar = new CBar(pGraphicDev);

	if (FAILED(pBar->Ready_GameObject(_eMaterial, vPos)))
	{
		Safe_Release(pBar);
		MSG_BOX("pBar Create Failed");
		return nullptr;
	}
