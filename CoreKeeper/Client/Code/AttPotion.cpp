#include "pch.h"
#include "..\Header\AttPotion.h"
#include "Export_System.h"
#include "Export_Utility.h"

CAttPotion::CAttPotion(LPDIRECT3DDEVICE9 pGraphicDev)
	: CItem(pGraphicDev)
{
	m_eItemNum = ITEM_POTION_ATT;

	m_tStat.iAttack = 10;
}

CAttPotion::~CAttPotion()
{
}

HRESULT CAttPotion::Ready_GameObject(_vec3 vPos)
{
...
_int CAttPotion::Update_GameObject(const _float& fTimeDelta)
{
...
void CAttPotion::LateUpdate_GameObject()
{
	Engine::CGameObject::LateUpdate_GameObject();
}

void CAttPotion::Render_GameObject()
{
...
HRESULT CAttPotion::Add_Component()
{
...
CAttPotion* CAttPotion::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CAttPotion* pAttPotion = new CAttPotion(pGraphicDev);

	if (FAILED(pAttPotion->Ready_GameObject(vPos)))
	{
		Safe_Release(pAttPotion);
		MSG_BOX("pAttPotion Create Failed");
		return nullptr;
	}
