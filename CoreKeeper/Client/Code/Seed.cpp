...
HRESULT CSeed::Ready_GameObject(ITEMNUM _eItemNum, _vec3 vPos)
{
	...
	case ITEM_BERRY_SEED:
		m_wItemName = L"원기 베리 씨앗";
		m_wItemExplain[0] = L"심으면 건강한 과일로 자라납니다.";
		break;
	case ITEM_PEPPER_SEED:
		m_wItemName = L"폭탄 후추 씨앗";
		m_wItemExplain[0] = L"심으면 매운 채소로 자라납니다.";
		break;
	case ITEM_CARROT_SEED:
		m_wItemName = L"돌당근 씨앗";
		m_wItemExplain[0] = L"돌 속에서만 자라나는 튼튼한 씨앗입니다.";
		break;
	case ITEM_FIBER_SEED:
		m_wItemName = L"유충 케이폭 씨앗";
		m_wItemExplain[0] = L"수확하면 섬유질이 나오는 식물로 자라납니다.";
		break;
	}
...
HRESULT CSeed::Add_Component()
{
	...
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_SeedTexture"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
