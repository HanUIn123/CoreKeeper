	...
	if (m_bActive)
	{
		...
		if (g_bIsTopCamera)
		{
			...
			case FRONT:
				...
				break;
			case BACK:
				m_pAnimatorCom->Set_CurState(SHOOT, 8, 11, 4);
				break;
			case RIGHT:
			case LEFT:
				m_pAnimatorCom->Set_CurState(SHOOT, 4, 7, 4);
				break;
			}
		}
		else
...
HRESULT CArrow::Add_Component()
{
	...
	pComponent = m_pBufferCom = dynamic_cast<CAnimTex*>(Engine::Clone_Proto(L"Proto_ArrowAnimTex"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].insert({ L"Com_Buffer", pComponent });

	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_ArrowTexture"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
