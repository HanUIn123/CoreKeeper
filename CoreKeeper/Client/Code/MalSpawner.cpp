#include "pch.h"
#include "..\Header\MalSpawner.h"
#include "Export_System.h"
#include "Export_Utility.h"

CMalSpawner::CMalSpawner(LPDIRECT3DDEVICE9 pGraphicDev)
	: CItem(pGraphicDev)
{
	m_eItemNum = ITEM_MAL_SPAWNER;
}

CMalSpawner::~CMalSpawner()
{
}

HRESULT CMalSpawner::Ready_GameObject(_vec3 vPos)
{
...
_int CMalSpawner::Update_GameObject(const _float& fTimeDelta)
{
...
void CMalSpawner::LateUpdate_GameObject()
{
...
HRESULT CMalSpawner::Add_Component()
{
...
CMalSpawner* CMalSpawner::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CMalSpawner* pMalSpawner = new CMalSpawner(pGraphicDev);

	if (FAILED(pMalSpawner->Ready_GameObject(vPos)))
	{
		Safe_Release(pMalSpawner);
		MSG_BOX("pMalSpawner Create Failed");
		return nullptr;
	}
