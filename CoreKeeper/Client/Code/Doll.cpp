...
CDoll::CDoll(LPDIRECT3DDEVICE9 pGraphicDev)
	: CItem(pGraphicDev)
{
	m_eItemNum = ITEM_DOLL;
}
...
HRESULT CDoll::Ready_GameObject(_vec3 vPos)
{
	FAILED_CHECK_RETURN(Add_Component(), E_FAIL);

	m_wItemName = L"동굴인 인형";
	m_wItemExplain[0] = L"특수한 소재로 만들어진 인형입니다.";
	m_wItemExplain[1] = L"악기를 쥐어주면 연주를 한다고 합니다.";
...
_int CDoll::Update_GameObject(const _float& fTimeDelta)
{
...
void CDoll::LateUpdate_GameObject()
{
	Engine::CGameObject::LateUpdate_GameObject();
}

void CDoll::Render_GameObject()
{
...
CDoll* CDoll::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CDoll* pDoll = new CDoll(pGraphicDev);

	if (FAILED(pDoll->Ready_GameObject(vPos)))
	{
		Safe_Release(pDoll);
		MSG_BOX("pDoll Create Failed");
		return nullptr;
	}
