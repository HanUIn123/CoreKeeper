...
COre::COre(LPDIRECT3DDEVICE9 pGraphicDev)
	: CItem(pGraphicDev)
{
...
HRESULT COre::Ready_GameObject(MATERIAL _eMaterial, _vec3 vPos)
{
...
_int COre::Update_GameObject(const _float& fTimeDelta)
{
...
void COre::LateUpdate_GameObject()
{
	Engine::CGameObject::LateUpdate_GameObject();
}

void COre::Render_GameObject()
{
...
COre* COre::Create(LPDIRECT3DDEVICE9 pGraphicDev, MATERIAL _eMaterial, _vec3 vPos)
{
	COre* pOre = new COre(pGraphicDev);

	if (FAILED(pOre->Ready_GameObject(_eMaterial, vPos)))
	{
		Safe_Release(pOre);
		MSG_BOX("pOre Create Failed");
		return nullptr;
	}
