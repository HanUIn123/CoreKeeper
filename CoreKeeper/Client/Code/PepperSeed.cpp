#include "pch.h"
#include "..\Header\PepperSeed.h"
#include "Export_System.h"
#include "Export_Utility.h"

CPepperSeed::CPepperSeed(LPDIRECT3DDEVICE9 pGraphicDev)
	: CItem(pGraphicDev)
{
	m_eItemNum = ITEM_PEPPER_SEED;
}

CPepperSeed::~CPepperSeed()
{
}

HRESULT CPepperSeed::Ready_GameObject(_vec3 vPos)
{
...
_int CPepperSeed::Update_GameObject(const _float& fTimeDelta)
{
...
void CPepperSeed::LateUpdate_GameObject()
{
	Engine::CGameObject::LateUpdate_GameObject();
}

void CPepperSeed::Render_GameObject()
{
...
HRESULT CPepperSeed::Add_Component()
{
	CComponent* pComponent = NULL;

	pComponent = m_pBufferCom = dynamic_cast<CAnimTex*>(Engine::Clone_Proto(L"Proto_PepperSeedAnimTex"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].insert({ L"Com_Buffer", pComponent });

	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_PepperSeedTex"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CPepperSeed* CPepperSeed::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CPepperSeed* pPepperSeed = new CPepperSeed(pGraphicDev);

	if (FAILED(pPepperSeed->Ready_GameObject(vPos)))
	{
		Safe_Release(pPepperSeed);
		MSG_BOX("pPepperSeed Create Failed");
		return nullptr;
	}
