...
_uint CLoading::Loading_Stage()
{
	...
	FAILED_CHECK_RETURN(Engine::Ready_Proto(L"Proto_UIHealthTex", Engine::CAnimTex::Create(m_pGraphicDev, 11, 1)), E_FAIL);
	...
	FAILED_CHECK_RETURN(Engine::Ready_Proto(L"Proto_UIHealthTex", Engine::CTexture::Create(m_pGraphicDev, L"../Bin/Resource/Texture/UI/UI_Health_%d.png", TEX_NORMAL, 2)), E_FAIL);

	//COLLIDER
	//반지름 넣으면 됩니다. 콜라이더 RENDER 기능 아직 안됨 ㅠ
	FAILED_CHECK_RETURN(Engine::Ready_Proto(L"Proto_PlayerCollider", Engine::CCollider::Create(m_pGraphicDev, 1.0f)), E_FAIL);
	FAILED_CHECK_RETURN(Engine::Ready_Proto(L"Proto_ItemCollider", Engine::CCollider::Create(m_pGraphicDev, 1.0f)), E_FAIL);
