...
HRESULT CBox::Ready_GameObject(_vec3 vPos)
{
...
CBox* CBox::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CBox* pBox = new CBox(pGraphicDev);
	
	if (FAILED(pBox->Ready_GameObject(vPos)))
	{
		Safe_Release(pBox);
		MSG_BOX("pBox Create Failed");
		return nullptr;
	}
