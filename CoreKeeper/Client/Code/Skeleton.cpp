...
HRESULT CSkeleton::Ready_GameObject(_vec3 vPos)
{
...
HRESULT CSkeleton::Add_Component()
{
	...
	pComponent = m_pBufferCom = dynamic_cast<CObjectTex*>(Engine::Clone_Proto(L"Proto_SkeletonTex"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].insert({ L"Com_Buffer", pComponent });

	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_SkeletonTexture"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CSkeleton* CSkeleton::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CSkeleton* pCore = new CSkeleton(pGraphicDev);
