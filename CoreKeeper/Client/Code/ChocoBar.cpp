#include "pch.h"
#include "..\Header\ChocoBar.h"
#include "Export_System.h"
#include "Export_Utility.h"

CChocoBar::CChocoBar(LPDIRECT3DDEVICE9 pGraphicDev)
	: CItem(pGraphicDev)
{
	m_eItemNum = ITEM_CHOCOBAR;
}
...
HRESULT CChocoBar::Ready_GameObject(_vec3 vPos)
{
...
void CChocoBar::LateUpdate_GameObject()
{
	Engine::CGameObject::LateUpdate_GameObject();
}

void CChocoBar::Render_GameObject()
{
...
HRESULT CChocoBar::Add_Component()
{
	...
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_ChocoBarTexture"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CChocoBar* CChocoBar::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CChocoBar* pChocoBar = new CChocoBar(pGraphicDev);

	if (FAILED(pChocoBar->Ready_GameObject(vPos)))
	{
		Safe_Release(pChocoBar);
		MSG_BOX("pChocoBar Create Failed");
		return nullptr;
	}
