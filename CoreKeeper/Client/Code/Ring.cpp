...
HRESULT CRing::Ready_GameObject(MATERIAL _eMaterial, _vec3 vPos)
{
...
_int CRing::Update_GameObject(const _float& fTimeDelta)
{
...
void CRing::LateUpdate_GameObject()
{
	Engine::CGameObject::LateUpdate_GameObject();
}

void CRing::Render_GameObject()
{
...
HRESULT CRing::Add_Component()
{
	...
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_RingTexture"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CRing* CRing::Create(LPDIRECT3DDEVICE9 pGraphicDev, MATERIAL _eMaterial, _vec3 vPos)
{
	CRing* pRing = new CRing(pGraphicDev);

	if (FAILED(pRing->Ready_GameObject(_eMaterial, vPos)))
	{
		Safe_Release(pRing);
		MSG_BOX("pRing Create Failed");
		return nullptr;
	}
