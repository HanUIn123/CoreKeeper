...
HRESULT CLeg::Ready_GameObject(MATERIAL _eMaterial, _vec3 vPos)
{
...
_int CLeg::Update_GameObject(const _float& fTimeDelta)
{
...
void CLeg::LateUpdate_GameObject()
{
	Engine::CGameObject::LateUpdate_GameObject();
}

void CLeg::Render_GameObject()
{
...
HRESULT CLeg::Add_Component()
{
	...
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_LegTexture"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CLeg* CLeg::Create(LPDIRECT3DDEVICE9 pGraphicDev, MATERIAL _eMaterial, _vec3 vPos)
{
	CLeg* pLeg = new CLeg(pGraphicDev);

	if (FAILED(pLeg->Ready_GameObject(_eMaterial, vPos)))
	{
		Safe_Release(pLeg);
		MSG_BOX("pLeg Create Failed");
		return nullptr;
	}
