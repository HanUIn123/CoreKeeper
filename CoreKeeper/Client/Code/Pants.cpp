...
CPants::CPants(LPDIRECT3DDEVICE9 pGraphicDev) : Engine::CGameObject(pGraphicDev)
{
}
...
HRESULT CPants::Ready_GameObject()
{
...
_int CPants::Update_GameObject(const _float& fTimeDelta)
{
	Follow_Player();
...
void CPants::LateUpdate_GameObject()
{
...
HRESULT CPants::Add_Component()
{
	...
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_PlayerTex_Pants"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
void CPants::Follow_Player()
{
	...
	if (playerTransform)
	{
		...
		else
		{
			...
			m_pTransformCom->Set_Pos(vPlayerPos.x, vPlayerPos.y, vPlayerPos.z - vPlayerLook.z * 0.001f);
...
CPants* CPants::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CPants* pPants = new CPants(pGraphicDev);

	if (FAILED(pPants->Ready_GameObject()))
	{
		Safe_Release(pPants);
		MSG_BOX("pPants Create Failed");
		return nullptr;
	}
