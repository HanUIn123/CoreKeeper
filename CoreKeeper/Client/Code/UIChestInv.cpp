...
CUIChestInv::CUIChestInv(LPDIRECT3DDEVICE9 pGraphicDev)
	: Engine::CGameObject(pGraphicDev), m_iCurInv(0), m_bShow(false), m_bCollapse(false), m_bStay(false)

{
}

CUIChestInv::~CUIChestInv()
{
}

HRESULT CUIChestInv::Ready_GameObject(_vec2 vPos, _int _iIndex)
{
...
_int CUIChestInv::Update_GameObject(const _float& fTimeDelta)
{
	...
	if (m_bShow)
	{
		...
		if (Map_Picked(pt))
		{
			...
			{
				pItem = m_pChestInv->Get_Item(m_iIndex);
...
void CUIChestInv::LateUpdate_GameObject()
{
...
void CUIChestInv::Render_GameObject()
{
...
CUIChestInv* CUIChestInv::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec2 vPos, _int _iIndex)
{
