#include "pch.h"
#include "..\Header\Furnace.h"
#include "Export_System.h"
#include "Export_Utility.h"

CFurnace::CFurnace(LPDIRECT3DDEVICE9 pGraphicDev)
	: CItem(pGraphicDev)
{
	m_eItemNum = ITEM_FURNACE;
}
...
HRESULT CFurnace::Ready_GameObject(_vec3 vPos)
{
	...
	m_wItemName = L"용광로";
	m_wItemExplain[0] = L"기본적인 금속 광석을 녹이는 일반적인 용광로 입니다.";
...
_int CFurnace::Update_GameObject(const _float& fTimeDelta)
{
...
void CFurnace::LateUpdate_GameObject()
{
	Engine::CGameObject::LateUpdate_GameObject();
}

void CFurnace::Render_GameObject()
{
...
HRESULT CFurnace::Add_Component()
{
...
CFurnace* CFurnace::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CFurnace* pFurnace = new CFurnace(pGraphicDev);

	if (FAILED(pFurnace->Ready_GameObject(vPos)))
	{
		Safe_Release(pFurnace);
		MSG_BOX("pFurnace Create Failed");
		return nullptr;
	}
