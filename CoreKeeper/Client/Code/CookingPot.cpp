...
HRESULT CCookingPot::Ready_GameObject(_vec3 vPos)
{
	...
	m_wItemName = L"요리 솥";
	m_wItemExplain[0] = L"재료의 진가는 요리한 후에야 드러납니다!";
	m_wItemExplain[1] = L"재료를 합치고 어떤 음식이 나오는지 보세요!";
...
CCookingPot* CCookingPot::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CCookingPot* pCookingPot = new CCookingPot(pGraphicDev);

	if (FAILED(pCookingPot->Ready_GameObject(vPos)))
	{
		Safe_Release(pCookingPot);
		MSG_BOX("pCookingPot Create Failed");
		return nullptr;
	}
