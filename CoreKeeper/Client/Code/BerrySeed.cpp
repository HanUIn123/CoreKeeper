#include "pch.h"
#include "..\Header\BerrySeed.h"
#include "Export_System.h"
#include "Export_Utility.h"

CBerrySeed::CBerrySeed(LPDIRECT3DDEVICE9 pGraphicDev)
	: CItem(pGraphicDev)
{
	m_eItemNum = ITEM_BERRY_SEED;
}

CBerrySeed::~CBerrySeed()
{
}

HRESULT CBerrySeed::Ready_GameObject(_vec3 vPos)
{
...
_int CBerrySeed::Update_GameObject(const _float& fTimeDelta)
{
...
void CBerrySeed::LateUpdate_GameObject()
{
	Engine::CGameObject::LateUpdate_GameObject();
}

void CBerrySeed::Render_GameObject()
{
...
HRESULT CBerrySeed::Add_Component()
{
	CComponent* pComponent = NULL;

	pComponent = m_pBufferCom = dynamic_cast<CAnimTex*>(Engine::Clone_Proto(L"Proto_BerrySeedAnimTex"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].insert({ L"Com_Buffer", pComponent });

	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_BerrySeedTex"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CBerrySeed* CBerrySeed::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CBerrySeed* pBerrySeed = new CBerrySeed(pGraphicDev);

	if (FAILED(pBerrySeed->Ready_GameObject(vPos)))
	{
		Safe_Release(pBerrySeed);
		MSG_BOX("pBerrySeed Create Failed");
		return nullptr;
	}
