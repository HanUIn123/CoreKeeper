    ...
    , m_iItvTile(1)
{
    ...
    if (!m_TextureInfo)
    {
        ...
        Resister_ImguiImage_ImGui(_pGraphicDevice, L"../Bin/Resource/Texture/ImGuiImage/Object/Azeos_Summon/Azeos_Summon.png", TEX_OBJECT, 1);
...
void CMapEditorScene::Piking_Tile()
{
    if (!m_bGuiHovered)
    {
        if (Engine::Get_DIMouseState(DIM_LB) & 0x80 && m_bSelectTile)
        {
            ...
            _int iCount = pow(m_iInstallTileIndexCount, 2);
...
{
    ImGui::SliderInt("Install Tile", &m_iInstallTileIndexCount, 1, 10);

    //ImGui::SliderInt("Tile Interval", &m_iItvTile, 2, 6);
}
    ...
    */
    ...
    for (_int i = 0; i < m_vecPlaceObject.size(); ++i)
    {
        ...
        if (auto pSlimeFloor = dynamic_cast<CSlimeFloor*>(m_vecPlaceObject[i]))
        {
            ...
            vTempTypeNumber = pSlimeFloor->Get_TileTypeIndex();
        ...
        else if (auto pMushroom = dynamic_cast<CMushroom*>(m_vecPlaceObject[i]))
        {
            ...
            vTempTypeNumber = pMushroom->Get_TileTypeIndex();
        ...
        else if (auto pAzeosPoop = dynamic_cast<CAzeosPoop*>(m_vecPlaceObject[i]))
        {
            ...
            vTempTypeNumber = pAzeosPoop->Get_TileTypeIndex();
            vTempObjectType = pAzeosPoop->Get_ObjectType();
        }
        else if (auto pAzeosFeather = dynamic_cast<CAzeosFeather*>(m_vecPlaceObject[i]))
        {
            vTempObjectPos = pAzeosFeather->Get_PoopPos();
